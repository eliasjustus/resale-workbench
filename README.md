...
The second command stages the exact `release-files.json` allowlist, builds its wheel/source distribution, runs the full suite against that candidate and creates an exact source ZIP only after success. It requires the local development tools `build`, `setuptools` and `wheel` (`python -m pip install build setuptools wheel`); the check itself does not install packages or contact sources. It refuses an existing destination and never publishes. Use a new destination for each candidate. Optional `--private-patterns PRIVATE_JSON` rejects owner-specific strings without including them in the candidate. `tools/stage_release.py` remains available for staging alone.
